...
Game.Grid.prototype = {
    ...
    drawNextPentomino: function () {
        ...
        _.each(this.nextPentomino.shape, function (row, y) {
            _.each(row, function (cell, x) {
                if (cell != 0) {
                    self.state.nextPentomino.key.rect(x * GRID_CELL_SIZE, y * GRID_CELL_SIZE, GRID_CELL_SIZE, GRID_CELL_SIZE, '#333');
                    self.state.nextPentomino.key.rect(x * GRID_CELL_SIZE + 0.5, y * GRID_CELL_SIZE + 0.5, GRID_CELL_SIZE - 1, GRID_CELL_SIZE - 1, self.nextPentomino.color);
                }
            });
        });
    },
    ...
    drawGrid: function (fallingGrid, skip) {
        ...
        _.each(this.grid, function (row, y) {
            ...
            _.each(row, function (cell, x) {
                if (cell != 0) {
                    self.pentominosData.rect(x * GRID_CELL_SIZE, y * GRID_CELL_SIZE, GRID_CELL_SIZE, GRID_CELL_SIZE, '#333');
                    self.pentominosData.rect(x * GRID_CELL_SIZE + 0.5, y * GRID_CELL_SIZE + 0.5, GRID_CELL_SIZE - 1, GRID_CELL_SIZE - 1, cell);
                }
                if (fallingGrid ? fallingGrid[y+5][x] != 0 : false) {
                    self.pentominosData.rect(x * GRID_CELL_SIZE, y * GRID_CELL_SIZE, GRID_CELL_SIZE, GRID_CELL_SIZE, '#333');
                    self.pentominosData.rect(x * GRID_CELL_SIZE + 0.5, y * GRID_CELL_SIZE + 0.5, GRID_CELL_SIZE - 1, GRID_CELL_SIZE - 1, fallingGrid[y+5][x]);
                }
            });
        });
